...
namespace CryptoScanner.DbSystem
{
    public class CoinDbController : IDbController<DbCoinModel>
    {
        ...
        public void Create(DbCoinModel model)
        {
            ...
            using (SqlCommand command = new SqlCommand("INSERT INTO " + model.Symbol + " VALUES (@TimeStamp, @Name, @Symbol, @Price)", _connection))
            {
                command.Parameters.Add("@TimeStamp", SqlDbType.DateTime).Value = model.TimeStamp;
                command.Parameters.Add("@Name", SqlDbType.VarChar).Value = model.Name;
                command.Parameters.Add("@Symbol", SqlDbType.VarChar).Value = model.Symbol;
                command.Parameters.Add("@Price", SqlDbType.Money).Value = model.Price;
        ...
        public void Delete(string table, int id)
        {
            ...
            //using (OdbcCommand command = new OdbcCommand("DELETE FROM " + table + " WHERE ID=" + id))
                using (SqlCommand command = new SqlCommand("DELETE FROM " + table + " WHERE ID=" + id))
                    command.ExecuteNonQuery();
                _connection.Close();
        ...
        public void CreateTable(string tablename)
        {
            ...
            using (SqlCommand command = new SqlCommand("CREATE TABLE " + tablename + " ( ID int PRIMARY KEY IDENTITY, TimeStamp datetime NOT NULL, Name varchar(30), Symbol varchar(10) NOT NULL, Price money)", _connection))
                command.ExecuteNonQuery();
        ...
        public bool CheckIfTableExists(string tablename)
        {
            ...
            using (SqlCommand command = new SqlCommand("SELECT count(*) as IsExists FROM dbo.sysobjects where id = object_id('[" + tablename + "]')", _connection))
